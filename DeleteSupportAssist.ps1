...
function Eliminar-DellSupportAssist {
    ...
    if ($SupportAssist) {
        Write-Host "Se encontró Dell SupportAssist. Procediendo con la desinstalación..." -ForegroundColor Green
        ...
        try {
            ...
            Write-Warning "Hubo un error durante la desinstalación: $_"
        }
    } else {
        Write-Warning "Dell SupportAssist no está instalado en este sistema."
    }
}
